...
// 2만건 1190ms
template<typename T1>
void sort_Select(T1 *&arr, const unsigned int length) {
	T1 t;
	unsigned int i, j;
...
// 2만건 433ms
template<typename T1>
void sort_Select2(T1 *&list, const unsigned int n) {
	int indexMin;
	T1 t;
	unsigned int i, j;
	for (i = 0; i < n - 1; i++)
...
// 2만건 1117ms
template<typename T1>
void sort_Bubble(T1 *&list, const unsigned int c) {
	T1 t = 0;
	unsigned int loop, i;
...
// 2만건 1079ms
template<typename T1>
void sort_Bubble2(T1 *&arr, const unsigned int c) {
	T1 t;
...
// 2만건 825ms
template<typename T1>
void sort_Bubble3(T1 *&arr, const unsigned int c) {
	T1 t;
...
//2만건 286
template<typename T1>
void insertion_sort(T1 *&arr, int c)
{
	int i, j;
	T1 remember;
	for (i = 1; i < c; i++)
...
//2만건 162
template<typename T1>
void insertion_sort2(T1 *&data, int n)
{
	int i, j;
	T1 remember;
	i = n - 1;
...
// 이건 어떻게 쓰는것인가
template<typename T1>
void insertion_sort3(T1 begin, T1 end)
{
	T1 bond = begin;
	for (++bond; bond != end; ++bond)
	{
		typename std::iterator_traits<T1>::value_type key = *bond;
		T1 ins = bond;
		T1 pre = ins;
